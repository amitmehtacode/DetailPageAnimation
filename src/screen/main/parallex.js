...
const DEFAULT_NAVBAR_COLOR = '#26AEEB';
...
class ParallaxScrollView extends Component {
  ...
  handleScroll(event) {
    ...
    if (Platform.OS === 'ios' && event > 230) {
      this.setState({showTitle: true});
    }
    if (Platform.OS === 'ios' && event > 160) {
      ...
      <Animated.ScrollView
        ...
        onScroll={Animated.event(
          [{nativeEvent: {contentOffset: {y: scrollY}}}],
          {
            ...
            listener: event =>
              this.handleScroll(Math.floor(event.nativeEvent.contentOffset.y)),
          },
        )}
  ...
  render() {
    ...
    return (
      <View style={[styles.container, containerStyle]}>
        <StatusBar
          barStyle={this.state.showStatusBar ? 'dark-content' : 'light-content'}
          backgroundColor={'transparent'}
        />
